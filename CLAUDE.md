...
> Источник правды по приложению — этот файл, код и память проекта. Корневой `README.md`
> и `/CLAUDE.md` описывают Python-ядро (duq-core за nginx, домен `on-za-menya.online`,
> префикс `/duq`) — актуальны. Карта фич/тестов: волт на VPS `Coding/duq/Android.md`.
